...
summary(lm.fit.in<-lm(price ~ . ,data=total_train ))
...
total_train <- total_train %>% select(-pred)
total_test <- total_test %>% select(-pred)
rf<-randomForest(price~. ,total_train ,ntree=200,importance=T)
plot(rf)
...
nnet_md<-nnet(formula = price~. ,data= total_train ,size=5,linout = TRUE,rang = 0.1,skip=TRUE,maxit = ITERATION)
#sizt=은닉층 노드 수
...
boosting <- gbm(price~., data = total_train ,
